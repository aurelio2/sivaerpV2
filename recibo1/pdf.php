...
$emola_db = $res2['emola'];
$pos_db = $res2['pos'];
$recebido_db = $res2['valor_recebido'];
$troco_db = $res2['troco'];
...
if($nome_cliente_db==55){
    $pdf->Cell(60,4,'Nome: Balcao',0,1,'C');
}else{
...
$select=$pdo->prepare("select SUM(t_iva) as t_iva from tbl_invoice_details where invoice_id=$id");
$select->execute();
while($item=$select->fetch(PDO::FETCH_OBJ)){
$pdf->SetX(7);
$pdf->SetFont('courier','B',8);
$pdf->Cell(15,2,'IVA(16%)',0,0,'');   
$pdf->Cell(10,4,' ',0,0,'');
$pdf->Cell(68,2,number_format($item->t_iva,2),0,1,'C');
$pdf->Cell(0,2,'---------------------------------------',0,1,'C');
$pdf->SetX(7);
...
// Exibe CASH apenas se o valor for maior que zero
if ($cash_db > 0) {
    $pdf->Cell(15,2,'CASH',0,0,'');   
    $pdf->Cell(10,4,' ',0,0,'');
    $pdf->Cell(68,2,number_format($cash_db,2),0,1,'C');
    $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
    $pdf->SetX(7);
}

// Exibe M-PESA apenas se o valor for maior que zero
if ($mpesa_db > 0) {
    $pdf->Cell(15,2,'M-PESA',0,0,'');   
    $pdf->Cell(10,4,' ',0,0,'');
    $pdf->Cell(68,2,number_format($mpesa_db,2),0,1,'C');
    $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
    $pdf->SetX(7);
}

// Exibe E-MOLA apenas se o valor for maior que zero
if ($emola_db > 0) {
    $pdf->Cell(15,2,'E-MOLA',0,0,'');   
    $pdf->Cell(10,4,' ',0,0,'');
    $pdf->Cell(68,2,number_format($emola_db,2),0,1,'C');
    $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
    $pdf->SetX(7);
}

// Exibe POS apenas se o valor for maior que zero
if ($pos_db > 0) {
    $pdf->Cell(15,2,'POS',0,0,'');   
    $pdf->Cell(10,4,' ',0,0,'');
    $pdf->Cell(68,2,number_format($pos_db,2),0,1,'C');
    $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
    $pdf->SetX(7);
}

// Exibe RECEBIDO apenas se o valor for maior que zero
if ($recebido_db > 0) {
    $pdf->Cell(15,2,'RECEBIDO',0,0,'');   
    $pdf->Cell(10,4,' ',0,0,'');
    $pdf->Cell(68,2,number_format($recebido_db,2),0,1,'C');
    $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
    $pdf->SetX(7);

    // Exibe TROCO apenas se RECEBIDO tiver valor
    if ($troco_db > 0) {
        $pdf->Cell(15,2,'TROCO',0,0,'');   
        $pdf->Cell(10,4,' ',0,0,'');
        $pdf->Cell(68,2,number_format($troco_db,2),0,1,'C');
        $pdf->Cell(0,2,'---------------------------------------',0,1,'C');
        $pdf->SetX(7);
    }
}
