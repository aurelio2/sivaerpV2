...
$consul_s = $pdo->prepare("
    ...
    GROUP BY p.pname
");
$consul_s->execute(['data1' => $data1, 'data2' => $data2]);
...
$select=$pdo->prepare("SELECT sum(subtotal) as total,count(invoice_id) as invoice from tbl_invoice  where order_date BETWEEN '$data1' AND '$data2'");
$select->execute();
...
<br>
<p align="center">MAPA DE CONTROLE DE '.$nome_db.' DE MARGENS, REFERENTE AO DIA '.$data1.' ate DIA '.$data2.'</p>
<style>
table {
